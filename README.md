...
Windows забывает буфер после перезагрузки и после следующего копирования. Клип держит текст, заметки, код и снимки экрана в одном прозрачном окне: нажали на карточку - содержимое снова в буфере.
...
Записи лежат у вас: метаданные в `%APPDATA%\Klip\klip.db`, PNG в `%APPDATA%\Klip\images`. Телеметрии нет. В сеть Клип ходит только чтобы проверить GitHub Releases и скачать обновление.

## Возможности

| | |
| --- | --- |
| **Автозахват** | Текст и изображения, включая `Win + Shift + S`, сразу попадают в историю |
| **Одно нажатие** | Клик или Enter возвращает текст или изображение в системный буфер |
| **Изображения** | Миниатюры, полноразмерный просмотр и сохранение PNG |
...
**MSI** - `Klip-1.2.0-win-x64.msi`

Для тихой установки и корпоративных машин:

```powershell
msiexec /i Klip-1.2.0-win-x64.msi /qn
```
...
```powershell
Get-FileHash .\Klip-Setup-1.2.0.exe -Algorithm SHA256
```
...
копирование в Windows
  ...
  WM_CLIPBOARDUPDATE
        │
        ▼
 SQLite + PNG %APPDATA%\Klip
        │
        ▼
 окно / трей / поиск  ──►  SetDataObject ──► снова в буфер
```

Повтор того же текста или изображения подряд не плодит дубликаты. Если содержимое уже было раньше, Клип поднимает старую карточку. Собственная вставка в буфер помечается и не пишется второй раз. Незакреплённая история изображений ограничена 100 PNG по 25 МБ каждый.
...
Да. Снимки `Win + Shift + S` и другие изображения из буфера сохраняются локально как PNG. Один клик возвращает изображение в буфер, двойной открывает просмотр.
