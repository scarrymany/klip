...
Windows забывает буфер после перезагрузки и после следующего копирования. Клип держит историю, заметки и код в одном стеклянном окне: нажали на карточку — текст снова в буфере.

На Windows 11 окно рисуется через **Mica / Acrylic**. На Windows 10 остаётся тёмная панель без подделки прозрачности.

Данные лежат только у вас: `%APPDATA%\Klip\klip.db`. Сети нет.
...
**MSI** — `Klip-1.0.10-win-x64.msi`

Для тихой установки и корпоративных машин:

```powershell
msiexec /i Klip-1.0.10-win-x64.msi /qn
```
...
```powershell
Get-FileHash .\Klip-Setup-1.0.10.exe -Algorithm SHA256
```
...
Повтор того же текста подряд не плодит дубликаты. Собственная вставка в буфер помечается и не пишется второй раз.
...
Mica и Acrylic — API Windows 11. На десятке Клип не рисует фальшивый blur, а использует плотный тёмный фон. Так меньше артефактов и лучше читаемость.
...
Нет. Ни телеметрии, ни проверки обновлений.
